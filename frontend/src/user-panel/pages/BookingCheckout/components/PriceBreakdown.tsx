...
import { PaymentSummaryData } from '../types/checkout';
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-semibold text-slate-400">
                Transparent itemized fare statement
              </p>
            </div>
          </div>
        ...
        <div className="space-y-3 text-xs sm:text-sm font-semibold">
          <div className="flex justify-between items-center py-1">
            <div className="flex items-center gap-2 text-slate-600">
              ...
              <span>Base Fare ({paymentSummary.travelerCount} Travelers)</span>
            </div>
            <span className="font-extrabold text-[#0F172A]">
              ₹{paymentSummary.packageTotal.toLocaleString('en-IN')}
            </span>
          </div>

          <div className="flex justify-between items-center py-1 text-slate-600">
            <span>Platform Services & Tech Fee</span>
            <span className="font-extrabold text-[#0F172A]">
              ₹{paymentSummary.platformFees.toLocaleString('en-IN')}
            </span>
          </div>
          ...
          {paymentSummary.insurancePrice > 0 && (
            <div className="flex justify-between items-center py-1 text-sky-700 bg-sky-50/70 border border-sky-100 px-3 py-2 rounded-2xl">
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold">Travel Protection ({paymentSummary.travelerCount} × ₹499)</span>
              </div>
              <span className="font-extrabold text-sky-700">
                +₹{paymentSummary.insurancePrice.toLocaleString('en-IN')}
              </span>
            </div>
          )}
          ...
          <div className="flex justify-between items-center py-1 text-slate-600">
            ...
            <span className="font-extrabold text-[#0F172A]">
              +₹{paymentSummary.taxes.toLocaleString('en-IN')}
            </span>
          </div>
          ...
          {paymentSummary.discountAmount > 0 && (
            <div className="flex justify-between items-center bg-emerald-50/80 border border-emerald-200/80 px-3 py-2.5 rounded-2xl text-emerald-800">
              ...
              <span className="font-black text-emerald-700">
                -₹{paymentSummary.discountAmount.toLocaleString('en-IN')}
              </span>
            </div>
          )}
        </div>
