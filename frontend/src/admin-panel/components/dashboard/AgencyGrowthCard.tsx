...
import { GrowthMetric } from '../../types/dashboard';
import { CustomAreaChart } from '../common/CustomAreaChart';

interface AgencyGrowthCardProps {
  metric: GrowthMetric;
}

export const AgencyGrowthCard: React.FC<AgencyGrowthCardProps> = ({ metric }) => {
  ...
  return (
    ...
    >
      ...
      {/* SVG Smooth Chart */}
      <div className="mt-4">
        <CustomAreaChart data={metric.dataPoints} color="orange" />
      </div>
      ...
      <div className="mt-3 pt-2.5 border-t border-slate-50 flex items-center justify-between text-[11px] font-bold text-slate-400">
        ...
        <span className="text-orange-600 font-extrabold cursor-pointer hover:underline">View Agencies</span>
      </div>
    </motion.div>
  );
};
