...
import { NotificationKPIStats, NotificationKPICardItem } from '../../../types/notificationsManagement';

interface NotificationKPIStatsProps {
  stats: NotificationKPIStats;
  onCardClick?: (id: string) => void;
}

export const NotificationKPIStatsCards: React.FC<NotificationKPIStatsProps> = ({
  stats,
  onCardClick,
}) => {
  const getCardIcon = (type: NotificationKPICardItem['iconType']) => {
    switch (type) {
      case 'sent':
        return {
          icon: <Send className="w-4 h-4 text-[#6356E5]" />,
          bg: 'bg-purple-50',
          strokeColor: '#6356E5',
        };
      case 'delivery_rate':
  ...
  const cardsList: NotificationKPICardItem[] = [
    stats.sentToday,
    stats.deliveryRate,
    stats.openRate,
    stats.clickRate,
