...
export const AuditBottomWidgets: React.FC<AuditBottomWidgetsProps> = ({
  ...
  const totalEvents = distribution.reduce((acc, d) => acc + d.count, 0);
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 items-stretch select-none">
      ...
      <div className="bg-white rounded-3xl p-5 border border-slate-100/90 shadow-2xs flex flex-col justify-between">
        ...
        <div className="pt-2 border-t border-slate-100 text-[10px] font-semibold text-slate-400 text-center">
          {distribution.length > 0
            ? `${distribution[0]?.name} generates ${distribution[0]?.percentage}% of events`
            : 'Telemetry active across all modules'}
        </div>
      </div>
          ...
          {days.map((day, dIdx) => (
            <div key={day} className="flex items-center gap-1">
              ...
              <div className="grid grid-cols-7 gap-1 flex-1">
                {(heatmapMatrix[dIdx] || Array(7).fill(0)).map((val, hIdx) => (
                  <div
                    key={hIdx}
                    className="h-2.5 rounded-sm"
                    style={{
                      backgroundColor: `rgba(99, 86, 229, ${Math.max(0.08, val)})`,
                    }}
                  />
                ))}
              </div>
      ...
      <div className="bg-white rounded-3xl p-5 border border-slate-100/90 shadow-2xs flex flex-col justify-between">
                ...
                <div className="flex items-center justify-between text-xs">
                  <div className="flex items-center gap-1.5 min-w-0">
                    ...
                    {adm.avatar && adm.avatar.trim() !== '' ? (
                      ...
                      <div className="w-5 h-5 rounded-full bg-[#6356E5]/10 text-[#6356E5] text-[9px] font-bold flex items-center justify-center shrink-0">
                        {adm.name.charAt(0)}
                      </div>
                    )}
        ...
        <div className="pt-2 border-t border-slate-100 text-[10px] font-semibold text-slate-400 text-center">
          Ranked by total administrative actions
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl p-5 border border-slate-100/90 shadow-2xs flex flex-col justify-between">
        ...
        {securityAlerts.length === 0 ? (
          <div className="py-8 text-center text-xs text-slate-400 font-medium space-y-1">
            ...
            <p className="text-emerald-700 font-bold text-[11px]">System Secure</p>
            <p className="text-[10px]">No high-severity threats detected.</p>
          </div>
        ) : (
