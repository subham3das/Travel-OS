...
import { ArrowLeft, Share2, Save, CheckCircle2, Instagram, Facebook, Youtube, Linkedin, Globe } from 'lucide-react';
...
export const SocialMediaPage: React.FC = () => {
  ...
  const [links, setLinks] = useState(profile.social);
  const [isSaved, setIsSaved] = useState(false);
  const [isSaving, setIsSaving] = useState(false);

  useEffect(() => {
    if (profile.social) {
      setLinks(profile.social);
    }
  }, [profile.social]);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      await updateProfile({ social: links, website: links.website });
      setIsSaved(true);
      setTimeout(() => setIsSaved(false), 3000);
    } catch (err: any) {
      alert(err.message || 'Failed to update social media links.');
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-screen pb-24 md:pb-16">
        ...
        <div className="bg-white/95 backdrop-blur-md border-b border-slate-100 px-4 sm:px-6 py-3.5 flex items-center justify-between sticky top-[57px] sm:top-[65px] z-20 select-none">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base sm:text-lg font-black text-[#0F172A]">Social Media Handles</h2>
              <p className="text-[11px] font-semibold text-slate-400">Instagram, Facebook, YouTube & website links</p>
            </div>
          </div>
        </div>

        <main className="flex-1 px-4 py-6 sm:px-6 sm:py-8 max-w-3xl mx-auto w-full">
          {isLoading && !links.instagram && !links.website ? (
            <div className="flex flex-col items-center justify-center py-24 space-y-3">
              <div className="w-8 h-8 border-3 border-[#583BE8] border-t-transparent rounded-full animate-spin" />
              <p className="text-xs font-bold text-slate-400">Loading social links...</p>
            </div>
          ) : (
            <motion.form
            ...
            >
              ...
              <div className="space-y-3.5">
                <div>
                  <label className="text-slate-700 block mb-1 flex items-center gap-1.5">
                ...
                <div>
                  <label className="text-slate-700 block mb-1 flex items-center gap-1.5">
                    <Globe className="w-4 h-4 text-[#583BE8]" />
