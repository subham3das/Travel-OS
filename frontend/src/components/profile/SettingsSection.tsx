...
export const AccountSettingsList: React.FC = () => {
  ...
  const settingsItems = [
    { id: 'security', label: 'Privacy & Security', icon: <ShieldCheck className="w-5 h-5 text-emerald-600" /> },
    { id: 'payments', label: 'Payment Methods', icon: <CreditCard className="w-5 h-5 text-sky-600" /> },
    { id: 'language', label: 'Language & Region', icon: <Globe2 className="w-5 h-5 text-purple-600" /> },
    { id: 'help', label: 'Help & Support', icon: <HelpCircle className="w-5 h-5 text-amber-600" /> },
  ];
      ...
      <div className="divide-y divide-slate-100">
        {settingsItems.map((item) => (
          <div
            ...
            className="p-3 flex items-center justify-between gap-4 cursor-pointer rounded-xl hover:bg-slate-50 transition-colors"
          >
