...
export const FinalAgreementCard: React.FC = () => {
  const { draft, updateStep8 } = usePackageWizard();

  const isAgreed = draft?.step8?.finalAgreement ?? true;
  ...
  return (
    <div className="bg-white rounded-3xl p-5 border border-slate-100/90 shadow-[0_4px_20px_rgba(0,0,0,0.03)] space-y-2 select-none">
      <label className="text-sm font-extrabold text-[#0F172A]">
        Final Agreement <span className="text-rose-500">*</span>
      </label>
      ...
      <label className="flex items-start gap-3 cursor-pointer pt-1">
        <input
          type="checkbox"
          checked={isAgreed}
          onChange={(e) => updateStep8({ finalAgreement: e.target.checked })}
        ...
        <span className="text-xs font-semibold text-slate-600 leading-relaxed">
          I confirm all information is accurate and I am authorized to publish this package.
        </span>
      </label>
    </div>
  );
