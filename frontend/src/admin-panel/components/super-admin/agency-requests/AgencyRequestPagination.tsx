...
interface AgencyRequestPaginationProps {
  currentPage: number;
  totalPages: number;
  totalItems: number;
  itemsPerPage: number;
...
export const AgencyRequestPagination: React.FC<AgencyRequestPaginationProps> = ({
  currentPage = 1,
  totalPages = 4,
  totalItems = 34,
  itemsPerPage = 10,
  ...
  return (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 py-3 px-2 text-xs font-semibold text-slate-500 select-none">
      ...
      <div>
        ...
        <span className="font-extrabold text-[#0F172A]">{totalItems}</span> requests
      </div>
